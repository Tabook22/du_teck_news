...
def create_table():
    try:
        ...
        if conn:
            ...
            conn.execute('''CREATE TABLE IF NOT EXISTS tbl_advertisements (
                            id INTEGER PRIMARY KEY AUTOINCREMENT,
                            title TEXT NOT NULL,
...
# Create directory for ad images if it doesn't exist
os.makedirs('static/images/ads', exist_ok=True)
...
def control():
    ...
    # Get advertisements for the control panel
    advertisements = []
    conn = get_db_connection()
    if conn:
        try:
            advertisements = conn.execute("SELECT * FROM tbl_advertisements ORDER BY position, id").fetchall()
        except sqlite3.Error as e:
            print(f"Error fetching advertisements: {e}")
        finally:
            conn.close()
    
    # Render the control panel page with advertisements
    return render_template('control.html', 
                           ...
                           advertisements=advertisements)  # Add this line
...
# Add route for managing advertisements
@app.route('/manage-advertisements', methods=['POST'])
@login_required
@admin_required
def manage_advertisements():
    action = request.form.get('action')
    message = None
    ...
    try:
        if action == 'add':
            ...
            file_path = os.path.join('static/images/ads', filename)
            file.save(file_path)
            
            # Get form data
            title = request.form['ad_title']
            link = request.form['ad_link']
            position = request.form['ad_position']
            is_active = 1 if 'ad_active' in request.form else 0
            
            # Insert new advertisement
            conn.execute("""
                INSERT INTO tbl_advertisements (title, image_url, link_url, position, is_active)
                VALUES (?, ?, ?, ?, ?)
            """, (title, '/' + file_path, link, position, is_active))
            conn.commit()
            ...
            if 'ad_image' in request.files and request.files['ad_image'].filename != '':
                ...
                old_image = conn.execute("SELECT image_url FROM tbl_advertisements WHERE id = ?", (ad_id,)).fetchone()
                ...
                conn.execute("""
                    ...
                    WHERE id = ?
                """, (title, '/' + file_path, link, position, is_active, ad_id))
                
                # Delete old image if exists and not default
                if old_image and old_image['image_url'] and old_image['image_url'] != '/static/images/default_ad.jpg':
                    try:
                        # Remove leading slash for file system path
                        old_path = old_image['image_url'][1:] if old_image['image_url'].startswith('/') else old_image['image_url']
                ...
                conn.execute("""
                    UPDATE tbl_advertisements 
                    SET title = ?, link_url = ?, position = ?, is_active = ? 
                    WHERE id = ?
                """, (title, link, position, is_active, ad_id))
            ...
            ad = conn.execute("SELECT image_url FROM tbl_advertisements WHERE id = ?", (ad_id,)).fetchone()
            
            if ad:
                # Delete from database
                conn.execute("DELETE FROM tbl_advertisements WHERE id = ?", (ad_id,))
                conn.commit()
                
                # Delete image file if not default
                if ad['image_url'] and ad['image_url'] != '/static/images/default_ad.jpg':
                    try:
                        # Remove leading slash for file system path
                        file_path = ad['image_url'][1:] if ad['image_url'].startswith('/') else ad['image_url']
            ...
            # Get current status
            ad = conn.execute("SELECT is_active FROM tbl_advertisements WHERE id = ?", (ad_id,)).fetchone()
            
            if ad:
                # Toggle status
                new_status = 0 if ad['is_active'] == 1 else 1
                conn.execute("UPDATE tbl_advertisements SET is_active = ? WHERE id = ?", (new_status, ad_id))
                conn.commit()
